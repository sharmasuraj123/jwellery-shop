...
export default function ProductGrid({ products, onLike, onBuyNow }) {
  ...
  return (
    <section id="products" className="section-spacing bg-white" data-testid="product-grid-section">
      <div className="container-custom">
        ...
        {products.length === 0 ? (
          <div className="text-center py-20" data-testid="no-products-message">
            <p className="text-stone-500 text-lg">No products available yet. Check back soon!</p>
          </div>
        ) : (
            ...
            {products.map((product, index) => (
              ...
              >
                <div className="product-image-container">
                  ...
                  <button
                    ...
                    className={`absolute top-4 right-4 p-3 bg-white/90 backdrop-blur-sm rounded-full transition-all duration-300 hover:bg-white ${
                      likedProducts.has(product.id) ? 'text-red-500' : 'text-stone-400'
                    }`}
                ...
                <div className="mt-6 space-y-3">
                  <div>
                    <span className="text-xs uppercase tracking-widest text-stone-400">{product.category}</span>
                    <h3 className="text-lg font-medium text-stone-900 mt-1">{product.name}</h3>
                    <p className="text-sm text-stone-600 mt-1">{product.description}</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xl font-semibold text-stone-900">${product.price.toFixed(2)}</span>
